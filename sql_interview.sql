...
select id,isim from personel6 where id in(select id from isciler);
...
select max(maas) as ikinci_en_yuksek from isciler where maas<(select max(maas) from isciler)
